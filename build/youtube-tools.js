...
import { saveToFilesystem, createTimestampedFilename } from "./file-manager.js";
/**
 * Register YouTube caption processing tools
 */
export function registerYouTubeCaptionTools(server) {
    const logger = {
    ...
    server.registerTool("process-youtube-captions", {
        ...
        try {
            ...
            const cleanedTextPath = await saveToFilesystem(captionText, baseFilename, sessionFolder // Save to session folder, not main folder
            );
            ...
            if (includeRawCaptions) {
                ...
                rawVttPath = await saveToFilesystem(await import('fs').then(fs => fs.promises.readFile(vttPath, 'utf-8')), rawFilename, sessionFolder // Save to session folder
                );
            }
...
4. You have write permissions to the output directory`
    ...
    server.registerTool("download-youtube-captions", {
...
VTT file saved to: ${vttPath}`
    ...
    server.registerTool("clean-vtt-file", {
 ...
 */
function extractVideoTitle(vttPath) {
    try {
